...
module.exports = {
    name: "풀링",
    description: "유저들을 다른 음성통화방으로 옮길 수 있어요!",
    async execute(message, args) {
        let list = ""
        if (!message.member.permissions.has(Permissions.FLAGS.MOVE_MEMBERS)) return message.reply("권한이 부족하여 풀링을 진행할 수 없습니다!")
        ...
        embed.setTitle("유저를 정말로 풀링하시겠습니까?")
        ...
        collector.on("collect", async i => {
            if (i.customId == "check") {
                embed.setTitle("유저를 정상적으로 **풀링** 하였습니다!")
                ...
                if (i.customId == "cancel") {
                    embed.setTitle("풀링이 정상적으로 **취소** 되었습니다!")
                    embed.setColor("RED")
                    await i.update({ embeds: [embed], components: [] })
                    return
                }
